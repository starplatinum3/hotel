...
//	private final static Logger log = Logger.getLogger(KMedoidsClusterer.class);
	...
	public void randowClustering() {
		...
		log.info("");
		log.info("*******************************************");
		log.info(String.format("*********[%s]随机聚类器启动***********",
...
//		随机选择了 k个 记录
		...
		for (DataSetRecord record : centralRecords) {
			log.info(String.format("随机选中中心记录id:%d| 中心记录名字:%s| 用户数目:%d",
					record.getId(), record.getName(), record.getUsers().length));
		}
	...
	private void printClustersInfo(List<Cluster> tmpClusters) {
		...
		for (Cluster cluster : tmpClusters) {
			log.info(String.format("簇名:%s,中心点:%d,记录个数:%d",
					cluster.getClusterName(),cluster.getCentralRecord().getId(),cluster.getRecordsList().size()));
			StringBuffer recordIds = new StringBuffer();
			for (DataSetRecord record : cluster.getRecordsList()) {
				recordIds.append(record.getId()+",");
			}
			log.info(String.format("recordIds  [%s]", recordIds.toString()));
		}
		...
		for (Cluster cluster : tmpClusters) {
			for (DataSetRecord record : cluster.getRecordsList()) {
				...
				log.debug(String.format("记录%s| 变动后[->%s = %d]| 变动前[->%s = %d]| 变动代价:变动后-变动前 = %d", 
						record.getInfo(), result[1], newDistance,
						cluster.getCentralRecord().getName(),oldDistance, newDistance - oldDistance));
...
//		是一个标记吗 有啥用处吗 name   getMinDistance 的 name 遗传算法
		return result;
	}
