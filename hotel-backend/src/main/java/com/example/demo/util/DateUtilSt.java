...
//    public static final String Y = "yyyy";
    ...
    public static LocalDate stringToLocalDateManyFormat(String dateStr) {
//      todo  FORMAT_LIST
        for (String format : FORMAT_LIST) {
//            Date date = stringtoDate(dateStr, format);
            LocalDate date = stringToLocalDate(dateStr, format);
            if (date != null) return date;
        }
//        String num = StrUtils.getNum(dateStr);
        return null;
    }

    //    https://blog.csdn.net/panchang199266/article/details/95724991
    ...
    public static Date stringToDateManyFormat(String dateStr) {
//      todo  FORMAT_LIST
        for (String format : FORMAT_LIST) {
            Date date = stringtoDate(dateStr, format);
            if (date != null) return date;
...
//    public static Date stringToDateManyFormatSecTry(String dateStr) {
////        dateStr = dateStr.trim();
//        dateStr=dateStr.replace(" ","");
//        Date date = stringToDateManyFormat(dateStr);
////        年的那种 会直接用上的
////        会先用 2000年 ，而不是 2000年9月
//        if (date != null) return date;
////        光是一个数字的 Y 不是当作日期，前面的循环parse 不能算
//        dateStr = StrUtils.getNum(dateStr);
//
//        return stringtoDate(dateStr, Y);
////        return stringToDateManyFormat(dateStr);
//    }



//    public static LocalDate stringToLocalDateManyFormatSecTry(String dateStr) {
////        dateStr = dateStr.trim();
////        dateStr=dateStr.replace(" ","");
//////        Date date = stringToDateManyFormat(dateStr);
////        LocalDate date = stringToLocalDateManyFormat(dateStr);
//////        年的那种 会直接用上的
//////        会先用 2000年 ，而不是 2000年9月
////        if (date != null) return date;
////        dateStr = StrUtils.getNum(dateStr);
////        return stringToLocalDate(dateStr, Y);
////        return stringToDateManyFormat(dateStr);
//        return  date2LocalDate(stringToDateManyFormatSecTry(dateStr));
//
//    }
    ...
    public  static String nowTimeStr(){
        Date date = new Date();
      return   dateToString(date,DateUtilSt.ymdhmFile);
    }
...
//        return dateToString(today.getTime(), FORMAT_ONE);
    ...
    public static String getNowFileFormat() {
        Calendar today = Calendar.getInstance();
        return dateToString(today.getTime(), ymdhms2);
    }
...
//    	System.out.println(min);
        String settlementDate = DateUtilSt.dateToString(new Date(), "yyyy-MM-dd");
        long day = DateUtilSt.dayDiff(DateUtilSt.stringtoDate("2017-06-22", "yyyy-MM-dd"), DateUtilSt.stringtoDate(settlementDate, "yyyy-MM-dd"));
        if (day >= 0) {
        ...
        String Time = DateUtilSt.getCurrDate("HH");
        System.out.println(Time);

        String Time2 = DateUtilSt.getCurrDate("mm");
